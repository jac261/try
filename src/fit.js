...
(function () {
  ...
  function runLib(type, dur, pc) {
    ...
    const reps = T.clamp(Math.round((dur - 25) / 12), 2, 4); // Threshold
    return [st('Warm-up', 'warmup', time(15), P('easy')),
  ...
  function bikeLib(type, dur, pc) {
    ...
    const reps = T.clamp(Math.round((dur - 25) / 12), 3, 5); // Threshold
    return [st('Warm-up', 'warmup', time(15), W(0.55, 0.65)),
  ...
  T.FIT = { supports: supports, build: build, encode: encode, download: download, filename: filename };
})();
