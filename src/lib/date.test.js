...
/* daysBetween rounds an instant against local midnight, so a bare `new Date()`
   ...
   start to a calendar day via iso() — race chip and post-race edge catch,
   2026-07-30. These pin BOTH behaviours: the iso() form holds all day, and the
   raw-Date form keeps its documented trap (if that rounding ever changes, the
   call-site rule needs re-auditing, so the trap failing is a real signal). */
